...
| ID | Title | Size | Deps | Notes |
|----|-------|------|------|-------|
| **B6.1** | Remote kernel / distributed deployment | L | B0.1 | **In progress** (remote operation and a public multi-node client are integrated. Durable signed node identity, mTLS, generation-fenced drain/quarantine, constrained placement, restart ownership reconstruction, duplicate-owner checks, challenged single-authority membership/discovery, and an authority ownership-lease registry with monotonic tokens are implemented. Quorum/authority failover, destination-enforced ownership fencing, live certificate lifecycle, migration, global quotas, trust convergence, rolling upgrades, and partition/disaster qualification remain #122.) |
