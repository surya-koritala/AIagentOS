...
| ID | Title | Size | Deps | Notes |
|----|-------|------|------|-------|
| **B6.1** | Remote kernel / distributed deployment | L | B0.1 | **In progress** (remote operation and a public multi-node client are integrated. Durable identity, mTLS, drain/quarantine, constrained placement, challenged single-authority membership, monotonic ownership leases, pre-fenced exact-ID creation, durable partial-creation reconciliation, opt-in idle renewal/route health, destination mutation fencing, and fenced streaming are implemented. Quorum/authority failover, live certificate lifecycle, migration, global quotas, trust convergence, rolling upgrades, and partition/disaster qualification remain #122.) |
