...
| Feature-gated HTML/playwright helpers | browse/search/navigation helper functions | **Experimental** | **Trusted operator process; feature-dependent** | These are direct trusted-application helpers, not `ResourceProvider` implementations and not runtime discovery entries. HTTP fetches use strict HTTPS, no ambient proxy, no redirects, bounded strict-UTF-8 bodies, bounded projections, and redacted errors. Automation launches a unique private profile, denies downloads through CDP, bounds operations and returned data, removes URL credentials/query/fragment from projections, and reaps Chromium plus its profile at shutdown. Unit tests retain every contract, and an opt-in disposable Chromium fixture proves navigation, input-error redaction, screenshot bounds, download denial, cross-profile cookie isolation, and removal of both profiles. The protected extended-security workflow installs its Chromium revision from the frontend lockfile and retains exact-commit Linux evidence. Kernel egress policy, agent authorization, supported-platform live evidence beyond Linux, and independent review remain open, so these helpers are excluded from the v1 runtime support promise and do not substitute for the kernel network or browser boundary. |
