...
| ID | Title | Size | Deps | Notes |
|----|-------|------|------|-------|
| **B6.1** | Remote kernel / distributed deployment | L | B0.1 | **In progress** (remote operation and a public multi-node client are integrated. Durable identity, mTLS with live node-local certificate/trust reload and revocation, drain/quarantine, constrained placement, challenged single-authority membership, monotonic ownership leases, pre-fenced exact-ID creation, durable partial-creation reconciliation, opt-in idle renewal/route health, destination mutation fencing, fenced streaming, and a qualified durable OpenRaft storage-v2 substrate are implemented. The substrate is not a running quorum: authenticated Raft peer transport/election, authority mutation integration/failover, coordinated cluster-wide certificate rollout, migration, global quotas, trust convergence, rolling upgrades, and partition/disaster qualification remain #122.) |
