//! vLLM adapter (OpenAI-compatible chat completions) with retry and exponential backoff.
//!
//! Targets a self-hosted vLLM OpenAI-compatible server. Bearer auth is optional:
//! pass an empty API key to skip the `Authorization` header.
...
impl LlmSession for VllmSession {
...
#[async_trait::async_trait]
impl LlmProviderAdapter for VllmAdapter {
