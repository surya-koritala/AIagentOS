...
| ID | Title | Size | Deps | Notes |
|----|-------|------|------|-------|
| **B6.1** | Remote kernel / distributed deployment | L | B0.1 | **In progress** (remote operation and a public multi-node client are integrated. Durable identity, mTLS, drain/quarantine, constrained placement, restart reconstruction, challenged single-authority membership, monotonic ownership leases, and durable destination-enforced mutation fences are implemented. The client does not yet automate authority claim/fence/route publication, and fenced streaming remains disabled. Quorum/authority failover, live certificate lifecycle, migration, global quotas, trust convergence, rolling upgrades, and partition/disaster qualification remain #122.) |
