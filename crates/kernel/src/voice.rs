...
pub async fn text_to_speech(
    ...
    let audio = match config.tts_provider {
        TtsProvider::OpenAiTts => {
            openai_tts(text, config.voice_name.as_deref().unwrap_or("alloy")).await?
        }
...
async fn whisper_stt(audio: &[u8]) -> Result<String, String> {
    ...
    json["text"]
        .as_str()
        .map(|s| s.to_string())
        .ok_or("No text in response".into())
}
...
async fn azure_tts(text: &str, language: &str) -> Result<Vec<u8>, String> {
    ...
    let ssml = format!(
        "<speak version='1.0' xml:lang='{}'><voice name='en-US-JennyNeural'>{}</voice></speak>",
        language, text
    );
    let resp = client
        .post(format!(
