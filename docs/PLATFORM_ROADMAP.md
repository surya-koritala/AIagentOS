...
| ID | Title | Size | Deps | Notes |
|----|-------|------|------|-------|
| **B6.1** | Remote kernel / distributed deployment | L | B0.1 | **In progress** (remote operation and a public multi-node client are integrated. Durable identity, mTLS with live node-local certificate/trust reload and revocation, drain/quarantine, constrained placement, challenged single-authority membership, monotonic ownership leases, pre-fenced exact-ID creation, durable partial-creation reconciliation, opt-in idle renewal/route health, destination mutation fencing, fenced streaming, durable OpenRaft storage-v2, and an internal bounded mTLS Raft runtime with three-node election/replication/failover/restart regression coverage are implemented. `agent-server` does not start the Raft runtime and public authority mutations are not quorum-backed. Production wiring, safe membership changes, authority-term fencing, coordinated cluster-wide certificate rollout, migration, global quotas, trust convergence, rolling upgrades, and partition/disaster qualification remain #122.) |
