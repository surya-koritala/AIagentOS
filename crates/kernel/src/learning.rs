...
impl RuleStore {
    ...
    pub fn get_rules(&self, scope: Option<&RuleScope>) -> Vec<CorrectionRule> {
        ...
        match scope {
            ...
            Some(s) => rules
                .iter()
                .filter(|r| &r.scope == s || r.scope == RuleScope::Global)
                .cloned()
                .collect(),
        }
    }
    ...
    pub fn rules_as_prompt(&self, context: &str) -> Option<String> {
        ...
        let rules_text: Vec<String> = applicable
            .iter()
            .map(|r| format!("- When you encounter '{}': {}", r.trigger, r.correction))
            .collect();
        Some(format!(
            "IMPORTANT RULES (learned from previous corrections):\n{}",
            rules_text.join("\n")
        ))
    }
...
mod tests {
    ...
    fn add_and_find_rules() {
        ...
        store.add_rule(
            "python".into(),
            "Always use type hints".into(),
            RuleScope::Global,
        );
        store.add_rule(
            "rust".into(),
            "Prefer &str over String in function params".into(),
            RuleScope::Global,
        );
    ...
    fn rules_as_prompt_works() {
        ...
        store.add_rule(
            "code".into(),
            "Add comments to all functions".into(),
            RuleScope::Global,
        );
    ...
    fn scope_filtering() {
        ...
        store.add_rule(
            "global".into(),
            "applies everywhere".into(),
            RuleScope::Global,
        );
        store.add_rule(
            "project".into(),
            "only this project".into(),
            RuleScope::Project("myproject".into()),
        );
