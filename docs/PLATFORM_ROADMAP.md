...
| ID | Title | Size | Deps | Notes |
|----|-------|------|------|-------|
| **B6.1** | Remote kernel / distributed deployment | L | B0.1 | **In progress** (remote operation and a public multi-node client are integrated. Durable identity, mTLS, drain/quarantine, constrained placement, restart reconstruction, challenged single-authority membership, monotonic ownership leases, destination-enforced mutation fences, authority-backed route publication/refresh, and fenced streaming are implemented. Cross-database creation reconciliation, automatic idle renewal, quorum/authority failover, live certificate lifecycle, migration, global quotas, trust convergence, rolling upgrades, and partition/disaster qualification remain #122.) |
