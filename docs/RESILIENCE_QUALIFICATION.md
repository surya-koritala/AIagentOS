...
is the strict schema-v1 configuration for three initial scenarios.
...
- a 24-hour or explicitly agreed soak on the target deployment with retained
  RSS, task, descriptor, database/WAL, queue, and permit samples;
- disk-full, database-lock, sandbox-crash, network-partition, and cancellation-
  storm injection in addition to the provider outage implemented here;
