...
| Kernel `BuiltinAppProvider` + process backend | `launch` | **E2E verified** | Untrusted launch is supported only through the qualified Linux rootless digest-pinned container contract. Trusted operator launch is not an untrusted sandbox. Native process mode and macOS/Windows container execution remain unavailable. |
...
| Standalone `ApplicationProvider` (`resources` crate) | `launch` | **Experimental** | One-shot trusted-application helper only. It is not registered by the kernel and is not evidence of sandboxed command execution. `close`, `send_input`, and `read_output` are unavailable and typed unsupported. |
