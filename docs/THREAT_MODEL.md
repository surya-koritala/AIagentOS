...
| Caller cancellation detaches a blocking filesystem mutation or releases capacity early | The blocking worker owns its admission permit until real completion. Cancellation is propagated to capability-operation checkpoints before irreversible mutations and during bounded scans; a bounded foreground drain hands any still-running host syscall to a reaper without releasing its permit. Sandbox teardown serializes on the same operation lock. |
...
| Oversized or alternate HTTP parameters smuggle credentials, files, cookies, or another protocol | The kernel accepts a strict per-operation parameter shape, bounded URL and JSON body depth/nodes/bytes, and bounded UTF-8 responses. Headers, credentials, cookies, upload/download paths, WebSocket mode, URL fragments, and redirects fail closed. The standalone network compatibility provider advertises no operations. |
| Untrusted command escapes into the server process or smuggles ambient authority | Filesystem sandboxes deny commands; native/trusted host launch and standalone application launch fail closed. Linux `Container` mode requires a rootless daemon and locally verified digest-pinned image. It accepts only bounded literal argv, fixes cwd, forwards no caller environment/stdin, has no implicit shell, applies a 30-second timeout plus strict-UTF-8 output bounds, network-none, read-only root, capability drop, `no-new-privileges`, PID/memory/CPU limits, and label-scoped process-tree cleanup. macOS and Windows reject container mode. |
